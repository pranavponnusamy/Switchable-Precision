...
def expand_prefix_cache(prefix_cache, batch_size):
    """Expand a prefix cache to a given batch size, returning a proper DynamicCache object."""
    if prefix_cache is None:
        return None
    
    # Check if it's a DynamicCache (new transformers)
    if hasattr(prefix_cache, 'key_cache'):
        expanded_cache = DynamicCache()
        for layer_idx in range(len(prefix_cache.key_cache)):
            key = prefix_cache.key_cache[layer_idx].expand(batch_size, -1, -1, -1)
            value = prefix_cache.value_cache[layer_idx].expand(batch_size, -1, -1, -1)
            expanded_cache.update(key, value, layer_idx)
        return expanded_cache
    else:
        # Legacy format (list of tuples) - convert to DynamicCache
        expanded_cache = DynamicCache()
        for layer_idx in range(len(prefix_cache)):
            key = prefix_cache[layer_idx][0].expand(batch_size, -1, -1, -1)
            value = prefix_cache[layer_idx][1].expand(batch_size, -1, -1, -1)
            expanded_cache.update(key, value, layer_idx)
        return expanded_cache
            ...
            with torch.no_grad():
                ...
                if self.prefix_cache:
                    if not prefix_cache_batch or current_batch_size != search_batch_size:
                        prefix_cache_batch = expand_prefix_cache(self.prefix_cache, current_batch_size)
                ...
                with torch.no_grad():
                    ...
                    if self.draft_prefix_cache:
                        if not draft_prefix_cache_batch or batch_size != search_batch_size:
                            draft_prefix_cache_batch = expand_prefix_cache(self.draft_prefix_cache, batch_size)
