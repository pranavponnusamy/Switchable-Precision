...
lora.set_active_quant_config(precisions[0])
...
for epoch in range(num_epochs):
    ...
    if is_main_process(rank):
        lora.save_lora(model, f"{checkpoint_dir}/model_lora_{epoch}.pt", precisions)
        wandb.save(f"{checkpoint_dir}/model_lora_{epoch}.pt")
        print(f"Saved checkpoint: model_lora_{epoch}.pt", flush=True)
    if is_distributed:
        dist.barrier()
