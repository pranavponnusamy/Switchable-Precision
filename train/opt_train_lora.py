...
val_batch_size = 120
num_cycles = 4
checkpoint_dir = "opt_train/lora_checkpoints"
max_bit = 4
min_bit = 8
lr = 1e-4
LORA_R = 32
...
lora.apply_lora_to_model(
    model, l, r=LORA_R, alpha=LORA_ALPHA, lora_attention=True, lora_mlp=True
)

utils.uniform_quant_config(QUANT_CONFIGS, 8)
lora.set_active_quant_config(l[0]) # save as default
...
for epoch in range(num_epochs):
    ...
    if is_main_process(rank):
        lora.save_lora(model, f"{checkpoint_dir}/model_lora.pt", l)
        wandb.save(f"{checkpoint_dir}/model_lora.pt")
        print(f"Saved checkpoint: model_lora.pt", flush=True)
